""" QM - Select Stock Pick Chart 1.1
...
# Decompressing .pbz2 & convert to pickle:
data = bz2.BZ2File(fileStr1+fileStr2+'.pbz2', 'rb')
df_top_momentum_names = pickle.load(data)
# df_top_momentum_names = pd.read_csv(fileStr1+fileStr2+'.csv', parse_dates=True, index_col='Unnamed: 0')

# df_top_momentum_names.tail()
####################################################################################################
...
def get_stk_history_rank(ticker,df_top_momentum_names,run_list,df_list):
...
if selectList not in holdingList.keys():
    ...
    get_stk_history_rank(ticker,df_top_momentum_names,run_list,df_list)
else:
